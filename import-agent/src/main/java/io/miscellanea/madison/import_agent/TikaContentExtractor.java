...
public class TikaContentExtractor implements ContentExtractor {
    ...
    // Constructors
    @Inject
    public TikaContentExtractor(@NotNull ImportAgentConfig config, @NotNull ResteasyClient client) {
        this.client = client;
        this.config = config;
    }
            ...
            try (Response response =
                ...
                if (response.getStatus() == HttpStatus.SC_OK) {
                    ...
                    throw new ContentException(
                            "Tika was unable to extract document conent (SC = " + response.getStatus() + ").");
                }
            } catch (Exception e) {
