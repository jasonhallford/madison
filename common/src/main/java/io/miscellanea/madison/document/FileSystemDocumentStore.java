...
public class FileSystemDocumentStore implements DocumentStore {
    ...
    @Override
    public void storeSource(@NotNull Fingerprint fingerprint, URL source) throws ContentException {
        ...
        try (BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream(sourcePath.toFile()))) {
            try (InputStream in = source.openStream()) {
                in.transferTo(out);
                out.flush();
            }
        } catch (Exception e) {
            throw new ContentException("Unable to write source for " +
                    fingerprint + " to store.", e);
        }

        logger.info("Document source with fingerprint '{}' written to content store at location '{}'.",
                fingerprint, sourcePath);
    }

    @Override
    public void storeThumbnail(@NotNull Fingerprint fingerprint, BufferedImage thumbnail) throws ContentException {
        ...
        try {
            ...
            logger.debug("Successfully wrote thumbnail for {} to store at '{}'.", fingerprint,
                    thumbnailPath);
        } catch (Exception e) {
    ...
    public void storeText(@NotNull Fingerprint fingerprint, String text) throws ContentException {
        ...
        logger.debug("Writing compressed text for {} to store at location '{}'.", fingerprint, contentPath);
        ...
        try (GZIPOutputStream out = new GZIPOutputStream(new BufferedOutputStream(new FileOutputStream(contentPath.toFile())))) {
            ...
            throw new ContentException("Unable to write compressed text for " + fingerprint +
                    " to store.", e);
        }
    }
    ...
    public synchronized boolean delete(@NotNull Fingerprint fingerprint) {
        ...
        try {
            ...
            logger.error("Unable to delete document with fingerprint " + fingerprint +
                    "; returning false.", e);
        }
    ...
    public Status status(@NotNull Fingerprint fingerprint) {
        Function<String, ComponentStatus> componentExists = (String component) -> {
            try {
    ...
    private URL getContentUrl(@NotNull Fingerprint fingerprint, String component) {
        ...
        if (contentPath.toFile().exists()) {
            try {
                ...
                logger.debug("URL for content with document fingerprint '{}' is '{}'.", fingerprint,
    ...
    private Path componentPath(Fingerprint fingerprint, String component) {
        return switch (component) {
            case SOURCE_COMPONENT -> this.documentRoot.resolve(fingerprint +
                    DOCUMENT_COMPONENT_EXTENSION);
            case THUMBNAIL_COMPONENT -> this.thumbnailRoot.resolve(fingerprint +
                    THUMBNAIL_COMPONENT_EXTENSION);
            case TEXT_COMPONENT -> this.textRoot.resolve(fingerprint +
